...
def LSUVinit(model,
    ...
    with torch.inference_mode():
        ...
        for layer_idx in range(gg['total_fc_conv_layers']):
            ...
            gg['act_dict'] = None
            if transformer:
                outputs = model.forward(input_ids=data_dev["input_ids"], attention_mask=data_dev["attention_mask"], token_type_ids=data_dev["token_type_ids"])
            else:
                outputs = model.forward(data_dev)
            if gg['act_dict'] is not None:
                ...
                while (abs(current_std - needed_std) > std_tol):
                    ...
                    if transformer:
                        _ = model.forward(input_ids=data_dev["input_ids"], attention_mask=data_dev["attention_mask"], token_type_ids=data_dev["token_type_ids"])
                    else:
                        _ = model.forward(data_dev)
                    current_std = gg['act_dict'].std()
                    current_mean = gg['act_dict'].mean()
